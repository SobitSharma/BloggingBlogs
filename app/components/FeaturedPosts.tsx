...
function FeaturedPostsSuspense() {
  const [data] = trpc.userPosts.getMany.useSuspenseQuery();
...
function PostCard({ post }: PostCardProps) {
  ...
  const remove = trpc.userPosts.remove.useMutation({
    onSuccess:()=>{
  ...
  return (
    <div>
      <DropdownMenu>
        <DropdownMenuTrigger disabled={remove.isPending}><Ellipsis/></DropdownMenuTrigger>
        <DropdownMenuContent>
          ...
          <DropdownMenuItem onClick={()=>remove.mutate({postId:post.id})}>Delete</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
